...
const core_1 = require("@wordle-duel/core");
const fs_1 = require("fs");
const path_1 = require("path");
const matchmaking_1 = require("./matchmaking");
const auth_1 = require("./auth");
const allowedWords = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/allowed.json'), 'utf-8'));
const solutionWords = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions.json'), 'utf-8'));
...
const legacySolutionWords = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions-legacy.json'), 'utf-8'));
(0, core_1.initDictionary)(allowedWords, solutionWords, legacySolutionWords);
...
const allowed6Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/allowed-6.json'), 'utf-8'));
const solution6Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions-6.json'), 'utf-8'));
const legacySolution6Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions-6-legacy.json'), 'utf-8'));
const allowed7Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/allowed-7.json'), 'utf-8'));
const solution7Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions-7.json'), 'utf-8'));
const legacySolution7Words = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/solutions-7-legacy.json'), 'utf-8'));
(0, core_1.initDictionaryForLength)(6, allowed6Words, solution6Words, legacySolution6Words);
(0, core_1.initDictionaryForLength)(7, allowed7Words, solution7Words, legacySolution7Words);
let properNoundlePuzzles = [];
try {
    properNoundlePuzzles = JSON.parse((0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../../web/data/propernoundle-puzzles.json'), 'utf-8'));
}
catch {
    console.warn('ProperNoundle puzzles not found, VS mode for ProperNoundle will not work');
}
function selectProperNoundlePuzzle(seed) {
    let hash = 0;
...
const MODE_BOARD_COUNT = {
    [core_1.GameMode.DUEL]: 1,
    [core_1.GameMode.QUORDLE]: 4,
    [core_1.GameMode.OCTORDLE]: 8,
    [core_1.GameMode.SEQUENCE]: 4,
    [core_1.GameMode.RESCUE]: 4,
    [core_1.GameMode.GAUNTLET]: 21,
    [core_1.GameMode.MULTI_DUEL]: 2,
    [core_1.GameMode.TOURNAMENT]: 1,
    [core_1.GameMode.PROPERNOUNDLE]: 1,
    [core_1.GameMode.DUEL_6]: 1,
    [core_1.GameMode.DUEL_7]: 1
};
...
const APPLY_TO_ALL_MODES = new Set([core_1.GameMode.QUORDLE, core_1.GameMode.OCTORDLE, core_1.GameMode.RESCUE]);
const MODE_MAX_GUESSES = {
    [core_1.GameMode.DUEL]: 6,
    [core_1.GameMode.QUORDLE]: 9,
    [core_1.GameMode.OCTORDLE]: 13,
    [core_1.GameMode.SEQUENCE]: 10,
    [core_1.GameMode.RESCUE]: 6,
    [core_1.GameMode.GAUNTLET]: 50,
    [core_1.GameMode.MULTI_DUEL]: 6,
    [core_1.GameMode.TOURNAMENT]: 6,
    [core_1.GameMode.PROPERNOUNDLE]: 6,
    [core_1.GameMode.DUEL_6]: 7,
    [core_1.GameMode.DUEL_7]: 8
};
...
const MODE_WORD_LENGTH = {
    [core_1.GameMode.DUEL_6]: 6,
    [core_1.GameMode.DUEL_7]: 7
};
...
function generateSolutionsForMode(mode, seed, boardCount) {
    const wordLength = MODE_WORD_LENGTH[mode] || 5;
    return wordLength === 5
        ? (0, core_1.generateSolutionsFromSeed)(seed, boardCount)
        : (0, core_1.generateSolutionsFromSeedForLength)(seed, boardCount, wordLength);
}
...
httpServer.on('request', (req, res) => {
    ...
    if (req.url.startsWith('/presence')) {
        ...
        const uniquePresence = new Set();
        io.sockets.sockets.forEach((sock) => {
            const id = sock.data.presenceId ?? sock.id;
    ...
    if (req.url.startsWith('/debug/queue')) {
        const sockets = [];
        io.sockets.sockets.forEach((sock) => {
            sockets.push({ id: sock.id, presence: sock.data.presenceId ?? null });
        });
        res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify({
            now: Date.now(),
    ...
    if (req.url.startsWith('/vs/counts')) {
        ...
        for (const m of matches.values()) {
            if (!m.ended)
                playing[m.mode] = (playing[m.mode] ?? 0) + 2;
        } // 2 players/match
        res.writeHead(200, {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': allowed,
...
function cleanupMatch(matchId) {
    ...
    if (m) {
        ...
        if (m.resumePending) {
            for (const t of m.resumePending.values())
                clearTimeout(t);
            m.resumePending.clear();
        }
        if (playerToMatch.get(m.player1.id) === matchId)
            playerToMatch.delete(m.player1.id);
        if (playerToMatch.get(m.player2.id) === matchId)
...
const submittedWordsByMatch = new Map();
...
const vsLog = [];
function logVS(ev, who, detail) {
    vsLog.push({ t: Date.now(), ev, who, detail });
    if (vsLog.length > 300)
        vsLog.splice(0, vsLog.length - 300);
}
...
io.use(async (socket, next) => {
    ...
    if (raw) {
        const verified = await (0, auth_1.verifyAccessToken)(raw);
        if (!verified)
            return next(new Error('invalid token'));
        socket.data.verifiedUserId = verified.userId;
    }
    next();
});
io.on('connection', (socket) => {
    ...
    const presenceId = verifiedUserId
        ? `u:${verifiedUserId}`
        : socket.handshake.auth?.presenceId;
    if (presenceId)
    ...
    if (presenceId && pendingReconnects.has(presenceId)) {
        ...
        if (match) {
            ...
            const resumeTimer = setTimeout(() => {
                ...
                if (pState.status === core_1.GameStatus.PLAYING || oState.status === core_1.GameStatus.PLAYING) {
                    pState.status = core_1.GameStatus.ABANDONED;
            ...
            (match.resumePending ?? (match.resumePending = new Map())).set(playerId, resumeTimer);
    ...
    socket.on('join_queue', ({ mode, dailySeed, inviteCode }) => {
        ...
        if (inviteCode) {
            ...
            for (const [code, lobby] of privateLobbies) {
                ...
                if (lobby.entry.player.id === playerId || sameUser)
                    privateLobbies.delete(code);
                else if (lobby.expiresAt < Date.now())
                    privateLobbies.delete(code);
            }
            ...
            const isSelf = existing != null && (existing.entry.player.id === playerId ||
    ...
    socket.on('leave_queue', () => {
        ...
        for (const [code, lobby] of privateLobbies) {
            if (lobby.entry.player.id === playerId)
                privateLobbies.delete(code);
        }
    });
    socket.on('submit_guess', ({ guess, boardIndex = 0 }) => {
        ...
        if (match.mode !== core_1.GameMode.PROPERNOUNDLE) {
        ...
        if (log)
            (isPlayer1 ? log.p1 : log.p2).push({ boardIndex, guess: guess.toUpperCase() });
        ...
        if (playerState.status !== core_1.GameStatus.PLAYING) {
        ...
        const normalizedGuess = match.mode === core_1.GameMode.PROPERNOUNDLE
            ? guess.toLowerCase().replace(/[^a-z]/g, '')
            : guess.toUpperCase();
        const normalizedSolution = match.mode === core_1.GameMode.PROPERNOUNDLE
        ...
        if (isCorrect) {
            ...
            (playerState.serverSolvedSet ?? (playerState.serverSolvedSet = new Set())).add(boardIndex);
            // For single-board modes, mark done
            if (MODE_BOARD_COUNT[match.mode] === 1) {
                playerState.status = core_1.GameStatus.WON;
                playerState.completedAt = Date.now();
            }
        }
        else if (playerState.guesses >= maxGuesses) {
            playerState.status = core_1.GameStatus.LOST;
            playerState.completedAt = Date.now();
        }
        // Evaluate tiles for opponent visualization (colors only, no letters)
        let latestGuess;
        try {
            const evalResult = (0, core_1.evaluateGuess)(normalizedSolution, normalizedGuess);
            latestGuess = {
                boardIndex,
                tiles: evalResult.tiles.map((t) => t.state),
            };
        }
        catch (e) {
        ...
        if (APPLY_TO_ALL_MODES.has(match.mode)) {
            ...
            for (let i = 0; i < match.solutions.length; i++) {
                if (solved?.has(i))
                    continue;
                try {
                    const ev = (0, core_1.evaluateGuess)(match.solutions[i].toUpperCase(), normalizedGuess);
                    latestGuesses.push({ boardIndex: i, tiles: ev.tiles.map((t) => t.state) });
                    // applyToAll: the one guess hit every board — record any it solved.
                    if (normalizedGuess === match.solutions[i].toUpperCase()) {
                        (playerState.serverSolvedSet ?? (playerState.serverSolvedSet = new Set())).add(i);
                    }
                }
        ...
        io.to(opponentSocket).emit('opponent_progress', {
            ...
            solved: playerState.status === core_1.GameStatus.WON,
        ...
        if (playerState.status !== core_1.GameStatus.PLAYING && opponentState.status !== core_1.GameStatus.PLAYING) {
            endMatch(matchId);
        }
        else if (playerState.status !== core_1.GameStatus.PLAYING) {
    ...
    socket.on('board_solved', ({ boardIndex }) => {
        ...
        const solvedSet = (playerState.solvedBoardSet ?? (playerState.solvedBoardSet = new Set()));
    ...
    socket.on('player_completed', ({ status, totalGuesses, timeMs }) => {
        const matchId = playerToMatch.get(playerId);
        if (!matchId) {
            logVS('player_completed_DROP', presenceId ?? socket.id, { status, reason: 'no matchId mapping' });
            return;
        }
        const match = matches.get(matchId);
        if (!match) {
            logVS('player_completed_DROP', presenceId ?? socket.id, { status, reason: 'match not found' });
            return;
        }
        confirmResume(match, playerId);
        const isPlayer1 = match.player1.id === playerId;
        ...
        if (claimedWon && match.mode !== core_1.GameMode.GAUNTLET) {
        ...
        playerState.status = claimedWon ? core_1.GameStatus.WON : core_1.GameStatus.LOST;
        playerState.completedAt = Date.now();
        ...
        logVS('player_completed', presenceId ?? socket.id, {
            ...
            willEnd: opponentState.status !== core_1.GameStatus.PLAYING,
        });
        // Both players must be done (playerState was just set to WON/LOST above)
        if (opponentState.status !== core_1.GameStatus.PLAYING) {
            endMatch(matchId);
        }
        else {
    ...
    socket.on('stage_completed', ({ stageIndex }) => {
        ...
        const history = submittedWordsByMatch.get(matchId);
        if (history)
            (isPlayer1 ? history.p1 : history.p2).clear();
    ...
    socket.on('abandon_match', () => {
        ...
        const isPlayer1 = match.player1.id === playerId;
        const playerState = isPlayer1 ? match.player1State : match.player2State;
        playerState.status = core_1.GameStatus.ABANDONED;
        playerState.completedAt = Date.now();
    ...
    socket.on('offer_rematch', () => {
        ...
        if (match.rematchOffers.size === 2) {
            ...
            const newSeed = (0, core_1.generateMatchSeed)();
            const boardCount = MODE_BOARD_COUNT[match.mode] || 1;
            let newSolutions;
            let newPuzzleMetadata;
            if (match.mode === core_1.GameMode.PROPERNOUNDLE && properNoundlePuzzles.length > 0) {
            ...
            const newMatch = {
                ...
                player1State: { guesses: 0, status: core_1.GameStatus.PLAYING, boardsSolved: 0, totalBoards: boardCount },
                player2State: { guesses: 0, status: core_1.GameStatus.PLAYING, boardsSolved: 0, totalBoards: boardCount },
    ...
    socket.on('disconnect', () => {
        ...
        if (matchId) {
            ...
            if (match) {
                ...
                const playerState = isPlayer1 ? match.player1State : match.player2State;
                const opponentState = isPlayer1 ? match.player2State : match.player1State;
                const stillLive = playerState.status === core_1.GameStatus.PLAYING || opponentState.status === core_1.GameStatus.PLAYING;
                ...
                if (stillLive && presenceId && playerState.status !== core_1.GameStatus.ABANDONED) {
...
function createMatch(player1, player2, mode, preferredSeed) {
    ...
    const seed = preferredSeed || (0, core_1.generateMatchSeed)();
    const boardCount = MODE_BOARD_COUNT[mode] || 1;
    const serverStartAt = Date.now() + MATCH_COUNTDOWN * 1000;
    let solutions;
    let puzzleMetadata;
    if (mode === core_1.GameMode.PROPERNOUNDLE && properNoundlePuzzles.length > 0) {
...
function armFinishTimeout(matchId) {
    const match = matches.get(matchId);
    if (!match || match.ended || match.finishTimer)
        return;
...
function timeoutMatch(matchId) {
    ...
    for (const state of [match.player1State, match.player2State]) {
        if (state.status === core_1.GameStatus.PLAYING) {
            state.status = core_1.GameStatus.LOST;
...
function confirmResume(match, playerId) {
...
function endMatch(matchId, forfeitBy, timedOut = false) {
    const match = matches.get(matchId);
    if (!match)
        return;
    ...
    if (match.capTimer) {
        clearTimeout(match.capTimer);
        match.capTimer = undefined;
    }
    if (match.finishTimer) {
        clearTimeout(match.finishTimer);
        match.finishTimer = undefined;
    }
    if (match.resumePending) {
        for (const t of match.resumePending.values())
            clearTimeout(t);
        match.resumePending.clear();
    }
    ...
    setTimeout(() => { if (matches.get(matchId)?.ended)
        cleanupMatch(matchId); }, ENDED_MATCH_TTL_MS);
    ...
    let winner = null;
    const p1Won = match.player1State.status === core_1.GameStatus.WON;
    const p2Won = match.player2State.status === core_1.GameStatus.WON;
    if (p1Won && !p2Won) {
        winner = 'player';
    }
    else if (p2Won && !p1Won) {
...
function userIdFromSocket(socketId) {
    const sock = io.sockets.sockets.get(socketId);
    const pid = sock?.data?.presenceId;
