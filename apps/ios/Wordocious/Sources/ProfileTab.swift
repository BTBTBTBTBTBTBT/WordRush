...
struct YourRecordsView: View {
    ...
    @State private var closest: (label: String, gap: String)?
    @State private var mode: GameMode = .duel
    @State private var loading = true
                    ...
                    if let closest {
                        HStack(spacing: 5) {
                            ...
                            (Text("You're ").font(Brand.font(11, .bold)).foregroundColor(Theme.textMuted)
                             + Text(closest.gap).font(Brand.font(11, .black)).foregroundColor(Theme.textPrimary)
                             + Text(" from the \(closest.label) record").font(Brand.font(11, .bold)).foregroundColor(Theme.textMuted))
                        }
                    }
                }
                .padding(14)
    ...
    private func load() async {
        ...
        // Closest all-time record (lower-is-better, solo, per mode) not yet held.
        var best: (label: String, gap: String)?
        ...
        for r in allRecs where r.holderId != uid && r.gameMode != nil && r.playType == "solo" {
            ...
            if r.recordType == "fastest_win", mine.fastestTime > 0, Double(mine.fastestTime) > r.recordValue {
                ...
                if gap < bestGap { bestGap = gap; best = ("\(modeTitle(r.gameMode!)) fastest win", "\(Int(gap))s away") }
                ...
                if gap < bestGap { bestGap = gap; best = ("\(modeTitle(r.gameMode!)) fewest guesses", "\(Int(gap)) away") }
            }
        }
        closest = best
        loading = false
