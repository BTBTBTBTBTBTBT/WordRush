...
struct InfoPage: View {
    ...
    private var sections: [InfoSection] {
        ...
        case .privacy: return [
            ...
            InfoSection("Information We Collect", bullets: ["Email address — provided during sign-up or via Google OAuth", "Username / display name — chosen when creating your profile", "Game statistics — scores, win/loss, completion times across all modes", "Streak data — daily streak counts and history", "Device and usage information — device type, usage patterns, and anonymous analytics", "Advertising identifier — free tier only, and only while ads are enabled: the Identifier for Advertisers, shared with our advertising partner only if you allow tracking when asked; Pro subscribers see no ads", "Crash and diagnostic data — when the app crashes or hits an error we record the fault, device model, OS version, and app version so we can fix it", "Push notification token — stored only if you enable notifications, so we can send daily-puzzle and match reminders"]),
