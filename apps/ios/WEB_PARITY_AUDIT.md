...
| `/profile` `/profile/[id]` | `app/profile/page.tsx` + `profile/` | `ProfileTab` + `ProfileDashboard` | 🟡 CORE + CHARTS matched: header+level tier+XP, Today's Dailies, GlobalSummaryRow, mode picker + per-mode stats, AND the per-mode dashboard — **guess distribution, 90-day activity calendar, solve-time line, time-of-day heatmap** (SwiftUI Charts + custom grids; `MatchStatsService` queries `matches`). Native solo games now write a `matches` row (`recordSoloMatch`) so charts populate from native play. + **mode picker rebuilt** (leading "All" chip + vertical chips w/ icon-tile/shortTitle/games-count) bound to optional selectedMode (nil = global "All" dashboard view). DEFERRED: top words, Pro insights (personal bests/nemesis/consistency), edit modal, social links, notification toggle |
