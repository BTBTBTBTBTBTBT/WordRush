...
const TONE = new Set(['MURDER','THEFT','KILLER','CORPSE','WEAPON','BULLET','POISON','SUICIDE','DEADLY','BLOODY','CANCER','RIFLE','KNIFE','PISTOL','GRAVE','DEATH','DYING','CRIME','DRUNK','ABUSE','TERROR','HATRED','TUMOR','TUMOUR',
  ...
  'GRISLY','KIDNAP','PSYCHO','AUTISM','GRIEF','CIGAR','CIGARS','GAMBLE','GASSED','BUTCH','TABOO','PAGAN','BRIBE','LIBEL','INMATE','WICKED','SLAYER','SNIPER','TYRANT','ASSAULT','MOLEST','INCEST','HAREM','GEISHA','PIMP','BROTHEL','CONDOM','TAMPON','ENEMA','RECTUM','ANAL','GENITAL','SCROTUM','PENIS','VAGINA','BREAST','BOOBS','NUDIST','STRIP','FETISH','ORGASM','AROUSE','SEDUCE','HORNY','LUSTY','RAUNCH','SMUT','PORN','DILDO','BONER','CRAPPY','PISSED','DAMN','HELL','BASTARD','BUGGER','SLUT','WHORE','HOOKER','TRAMP','JUNKIE','STONER','DEALER','CARTEL','MAFIA','THUG','GANG','GANGS','RIOT','LOOTER','FASCIST','NAZI','JIHAD','TERROR','SUICIDE','AUTISM','ASTHMA','DAMAGE','DEMENT','SENILE','CRIPPLE','LEPROSY','RABIES','HERPES','SYPHILIS','DIABETES','ANEMIA','ANEMIC','OBESE','ANOREXIA','BULIMIA','DYSLEXIA','EPILEPSY','PARALYSIS','AMPUTEE','COMA','MORBID','CORONER','HEARSE','CASKET','CREMATE','EULOGY','OBITUARY','MOURNER','WIDOWER','ORPHANED','STILLBORN','MISCARRY','ABORT','ABORTED','FETUS','SPERM','OVARY','UTERUS','MENSES','PUBERTY','PUBIC','GROIN','BUTTOCK','BUTTS','CRAP','PISS','TURD','POOP','FARTED','BURP','PUKE','VOMITED','BARF','SNOTTY','LOUSY','SCABBY','PIMPLY','ZITS','DANDRUFF','LICE','FLEAS','TICKS','MITES','COCKROACH','SEWER','CESSPIT','LATRINE','OUTHOUSE','BEDPAN','CATHETER','SUPPOSITORY','LAXATIVE','DIARRHEA','CONSTIPATE','HEMORRHOID','GONORRHEA','VENEREAL',
  'LIQUOR','DRINK','DRUNKS','BURIAL','BURIED','BURY','TRAUMA','DETOX','TOXIN','TOXIC','FILTHY','OUTLAW','GUILT','REHAB','UNICEF','ASYLUM','BIKINI','TAVERN','SALOON','LAGER','WINES','WINERY','BREWER','BREWERY','TEQUILA','WHISKEY','SHERRY','CIDER','BOURBON','MARTINI','ALCOHOL','HANGOVER','TIPSY','SOBER','BINGE','ADDICTED','OVERDOSE','NARCOTIC','OPIATE','STEROID','TOBACCO','NICOTINE','SMOKER','ASHTRAY','CIGARETTE','HOOKAH','VAPING','HOSPICE','CHEMO','BIOPSY','MALIGNANT','LESION','ULCER','GANGRENE','SEPSIS','TETANUS','TYPHOID','MALARIA','EBOLA','COVID','FEVERISH','INFIRM','BEDRIDDEN','SENILITY','LUNATIC','MANIAC','INSANE','CRAZED','DERANGED','PSYCHOTIC','BIPOLAR','ANXIETY','PHOBIA','PANIC','DESPAIR','MISERY','AGONY','TORMENT','ANGUISH','SUFFER','WEEPING','SOBBING','WAILING','TEARFUL','MOURNFUL','GRIEVE','BEREFT','LONELY','DIVORCED','CUSTODY','ALIMONY','EVICTED','HOMELESS','BEGGAR','PAUPER','BANKRUPT','FORECLOSE','LAYOFF','FIRED','SACKED','JOBLESS','WELFARE','DOLE','CERVIX','MARTYR','MARTYRS','DOPING','DOPED','PURGED','PURGE','ORGAN','ORGANS','OVARIES','BOWEL','BOWELS','BLADDER','KIDNEY','SPLEEN','TUMOR','TUMOUR','LUMPS','BRUISE','BRUISED','BLISTER','SCAB','SCABS','WOUNDED','BLEEDS','BLOODY','GORE','GORY','CARNAGE','MASSACRE','SLAUGHTER','BUTCHER','KILLING','SLAYING','STABBING','SHOOTING','BOMBING','HIJACK','RANSOM','KIDNAPS','ABDUCT','CAPTIVE','TORTURED','BEATEN','BATTERED','BULLIED','BULLY','BULLIES','TAUNT','MOCKED','SNEER','LOATHE','DESPISE','HATEFUL','SPITEFUL','VICIOUS','CRUEL','CRUELTY','SADIST','BRUTAL','SAVAGE']);
// The profanity set guards the ANSWER words too (the 6-letter production list carries at least one hard profanity; found 2026-09-23).
const pool = [...upperList('solutions.json'), ...upperList('solutions-6.json')].filter((w) => !never.has(w) && !TONE.has(w) && !hard.has(w) && anagramCount.get(sig(w)) === 1);
...
jokes.forEach((j, n) => {
  ...
  for (let t = 0; t < 4000 && !puzzle; t++) {
    ...
    puzzle = { id, holiday: j.holiday, words, final: { answer: j.final, pattern: j.final.split(" ").map((x) => x.length) }, tray, caption: j.caption, scene: j.scene, altText: j.altText };
  }
