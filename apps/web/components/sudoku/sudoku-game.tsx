...
export function SudokuGame({ isDaily = false }: SudokuGameProps) {
  ...
  return (
    ...
    >
      {showVictory && <VictoryAnimation onComplete={() => setShowVictory(false)} timeSeconds={elapsedSeconds} onPlayAgain={mode !== 'daily' && isPro ? () => startPractice(difficulty) : undefined} />}
      {showGameOver && <GameOverAnimation onComplete={() => setShowGameOver(false)} timeSeconds={elapsedSeconds} onPlayAgain={mode !== 'daily' && isPro ? () => startPractice(difficulty) : undefined} />}
