...
async function uploadAndBuildShareUrl(blob: Blob, input: ShareImageInput): Promise<string | null> {
  try {
    ...
    if (input.layout === 'multi') {
    ...
    } else if (input.layout === 'sudoku' || input.layout === 'regions' || input.layout === 'ladder' || input.layout === 'wordsearch' || input.layout === 'hub' || input.layout === 'cryptogram' || input.layout === 'groups') {
      ...
      if (input.layout === 'cryptogram') params.set('ck', String(input.checks));
