...
async function uploadAndBuildShareUrl(blob: Blob, input: ShareImageInput): Promise<string | null> {
  try {
    ...
    } else if (input.layout === 'sudoku' || input.layout === 'regions' || input.layout === 'ladder' || input.layout === 'wordsearch') {
      // Non-guess modes: the unfurl names score, time and mistakes / par / misses (§18d).
