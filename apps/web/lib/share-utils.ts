...
async function uploadAndBuildShareUrl(blob: Blob, input: ShareImageInput): Promise<string | null> {
  try {
    ...
    } else if (input.layout === 'sudoku' || input.layout === 'regions' || input.layout === 'ladder' || input.layout === 'wordsearch' || input.layout === 'hub' || input.layout === 'cryptogram' || input.layout === 'groups' || input.layout === 'crossword') {
