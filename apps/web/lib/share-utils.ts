...
async function uploadAndBuildShareUrl(blob: Blob, input: ShareImageInput): Promise<string | null> {
  try {
    ...
    if (input.layout === 'multi') {
    ...
    } else if (input.layout === 'sudoku' || input.layout === 'regions' || input.layout === 'ladder' || input.layout === 'wordsearch' || input.layout === 'hub' || input.layout === 'cryptogram' || input.layout === 'groups' || input.layout === 'crossword' || input.layout === 'scramble') {
      // Non-guess modes: the unfurl names score, time and mistakes / par / misses / rank (§18d).
      ...
      if (input.layout === 'cryptogram' || input.layout === 'crossword') params.set('ck', String(input.checks));
      if (input.layout === 'scramble') { params.set('ck', String(input.checks)); params.set('bs', String(input.solvedCount)); params.set('tb', '5'); }
      if (input.layout === 'groups') { params.set('mk', String(input.mistakes)); params.set('bs', String(input.solvedTiers.length)); params.set('tb', '4'); }
      if (input.layout === 'wordsearch') { params.set('bs', String(input.found.length)); params.set('tb', String(input.words.length)); }
      if (input.layout === 'hub') { params.set('pct', String(input.pct)); params.set('bs', String(input.wordsFound)); params.set('tb', String(input.wordCount)); }
    }
