...
function Bars({ points, color }: { points: { day: string; value: number }[]; color: string }) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center gap-2 text-xs font-black text-gray-400 uppercase tracking-wide"><TrendingUp className="w-3.5 h-3.5" /> DAU today</div>
          <p className="text-2xl font-black text-gray-900 mt-1">{todayDau}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center gap-2 text-xs font-black text-gray-400 uppercase tracking-wide"><Share2 className="w-3.5 h-3.5" /> Share rate (14d)</div>
          <p className="text-2xl font-black text-gray-900 mt-1">{totalGames14 ? Math.round((1000 * totalShares14) / totalGames14) / 10 : 0}%</p>
          <p className="text-xs font-bold text-gray-400">{totalShares14} shares / {totalGames14} games</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="text-xs font-black text-gray-400 uppercase tracking-wide">Pro conversion</div>
          <p className="text-2xl font-black text-gray-900 mt-1">{summary.proPct}%</p>
          <p className="text-xs font-bold text-gray-400">{summary.activePro} Pro / {summary.totalProfiles} accounts</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center gap-2 text-xs font-black text-gray-400 uppercase tracking-wide"><UserPlus className="w-3.5 h-3.5" /> Signups (7d)</div>
          <p className="text-2xl font-black text-gray-900 mt-1">{summary.signups7d}</p>
          <p className="text-xs font-bold text-gray-400">{summary.guestConversions} from guest play</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center gap-2 text-xs font-black text-gray-400 uppercase tracking-wide"><Flag className="w-3.5 h-3.5" /> Reports (30d)</div>
          <p className="text-2xl font-black text-gray-900 mt-1">{reports30}</p>
          <p className="text-xs font-bold text-gray-400">the quiet sixth number</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <Bars points={data.dau.map((d) => ({ day: d.day, value: d.dau }))} color="#7c3aed" />
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <p className="text-xs font-black text-gray-400 uppercase tracking-wide mb-3">Shares per day — last 14 days</p>
          <Bars points={data.shareRate.map((d) => ({ day: d.day, value: d.shares }))} color="#ec4899" />
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {([
          ['Shares by mode', data.shareBreakdown.byMode],
          ['Shares by surface', data.shareBreakdown.bySurface],
          ['Sign-in providers', data.providers],
