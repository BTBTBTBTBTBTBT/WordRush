import { GameMode, GameStatus, GAUNTLET_STAGES, GAUNTLET_TOTAL_SOLUTIONS } from './types';
...
export function initializeGame(seed, mode) {
    return createInitialState(seed, mode);
}
function createBoardState(solution, maxGuesses = 6) {
    return {
        solution,
        guesses: [],
        maxGuesses,
...
function createStageBoardsFromSolutions(seed, stage, solutions) {
...
function getStageSolutionSlice(allSolutions, stageIndex) {
    let offset = 0;
...
export function createInitialState(seed, mode) {
    let boardCount = 1;
    let maxGuesses = 6;
    switch (mode) {
        case GameMode.DUEL:
...
export function gameReducer(state, action) {
    switch (action.type) {
        ...
        case 'NEXT_STAGE': {
            ...
            const stageTimeMs = action.elapsedMs !== undefined && gauntlet.stageStartElapsedMs !== undefined
        ...
        case 'STEAL_GUESS': {
            ...
            const stolenBoards = state.boards.map(b => {
                if (b.status !== GameStatus.PLAYING)
                    return b;
        ...
        case 'BLACKOUT_RESTART': {
            ...
            if (!failedBoard || failedBoard.status !== GameStatus.LOST)
                return state;
            const gauntlet = state.gauntlet;
