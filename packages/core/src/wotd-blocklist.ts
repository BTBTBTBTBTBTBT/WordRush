 ...
 */
export const WOTD_BLOCKLIST: readonly string[] = [
  ...
  'OPIUM',
  'BOOZE',
  // Disease framed unpleasantly, or stigmatizing of people
  'LEPER',
  'TUMOR',
  'ULCER',
  // Definition in our dataset is a street-gang reference
  'BLOOD',
];

const BLOCKED = new Set(WOTD_BLOCKLIST);

/** True when a word must not be featured as Word of the Day. Case-insensitive. */
export function isBlockedWordOfDay(word: string): boolean {
  return BLOCKED.has(word.toUpperCase());
}
