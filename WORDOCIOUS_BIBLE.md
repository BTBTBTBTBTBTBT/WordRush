...
*2026-06-01 update (ads): **Ads wired on both clients.** WEB (AdSense, pub-3015627373086578): the interstitial-on-game-start (`AdGate`, 5s countdown → Continue, Pro-skip) + `ads.txt` + loader were already built; added the missing bottom `AdBanner` (free users, dormant until env set). Activation is config-only — set Vercel `NEXT_PUBLIC_ADS_ENABLED=true` + `NEXT_PUBLIC_ADSENSE_CLIENT_ID` + interstitial/banner slot IDs, and get the site AdSense-approved. NATIVE (AdMob, GoogleMobileAds 11.13): new `AdsManager` (SDK start + ATT + game-start interstitial) + `AdBannerContainer` (bottom banner via RootTabView safeAreaInset), gated to non-Pro; interstitial fires on `GameScreen.onAppear` and the timer starts only after dismissal. Uses Google TEST unit IDs — **verified live in the sim** (ATT prompt, test banner, test interstitial w/ skip-countdown). Swap real AdMob app/unit IDs per `apps/ios/ADMOB_SETUP.md`. Note: web AdSense can't do a true 30s-skippable video (that's the native AdMob interstitial/rewarded path).*
